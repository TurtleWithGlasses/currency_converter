...
def show_currency():
    input_rate = float(text.text())
    input_cur = in_combo.currentText()
    target_cur = target_combo.currentText()
    rate = get_currency(input_cur,target_cur)
    output = round(input_rate * rate, 2)
    message = f"{input_rate} {input_cur} is {output} {target_cur}"
